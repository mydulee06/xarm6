...
class XArmDriverRunner
{
    public:
        XArmDriverRunner(rclcpp::Node::SharedPtr& node, std::string &server_ip)
        {
            ...
            xarm_driver_.arm->set_cgpio_digital(0, 1);
        ...
        void _report_data_callback(XArmReportData *report_data_ptr)
        {
            ...
            xarm_state_msg_.voltages.resize(joint_num_);
            xarm_state_msg_.currents.resize(joint_num_);
            ...
            for(int i = 0; i < joint_num_; i++)
            {
                ...
                double d_v = report_data_ptr->voltages[i];
                double r_v;
                char str_v[8];
                sprintf(str_v, "%0.3f", d_v);
                sscanf(str_v, "%lf", &r_v);
                xarm_state_msg_.voltages[i] = r_v;

                double d_c = report_data_ptr->currents[i];
                double r_c;
                char str_c[8];
                sprintf(str_c, "%0.3f", d_c);
                sscanf(str_c, "%lf", &r_c);
                xarm_state_msg_.currents[i] = r_c;
            }
            for(int i = 0; i < 6; i++)
            {
            ...
            unsigned char sn[40];
            xarm_driver_.arm->core->get_robot_sn(sn);

            const unsigned char* unsignedChar = &sn[0];
            std::string sn_str = "";
            sn_str = static_cast<std::string>(reinterpret_cast<const char *>(unsignedChar));

            xarm_state_msg_.serial_number = sn_str;
            xarm_state_msg_.robot_ip = robot_ip;
