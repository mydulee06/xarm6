...
def generate_launch_description():
    ...
    xarm_moveit_realmove_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(PathJoinSubstitution([FindPackageShare('xarm_moveit_config'), 'launch', '_xarm_moveit_realmove_wo_rviz.launch.py'])),
        launch_arguments={
            'robot_ip': robot_ip,
    ...
    add_object = Node(
        ...
        prefix = ["bash -c 'sleep 10.0; $0 $@' "],
    )

    xarm_http_server = Node(
        name="xarm_http_server",
        package="xarm_network",
        executable="xarm_http_server",
    )

    xarm_http_client = Node(
        name="xarm_http_client",
        package="xarm_network",
        executable="xarm_http_client",
    )

    xarm_mqtt_pub = Node(
        name="xarm_mqtt_pub",
        package="xarm_network",
        executable="xarm_mqtt_pub",
    )
