...
private:
    void get_io_states_(const xarm_msgs::msg::CIOState::SharedPtr iostate);
    ...
    : node_(node)
{
    ...
    io_state_sub_ = node_->create_subscription<xarm_msgs::msg::CIOState>("/xarm/xarm_cgpio_states", 5, std::bind(&XArmPlannerRunner::get_io_states_, this, std::placeholders::_1));
...
void XArmPlannerRunner::get_io_states_(const xarm_msgs::msg::CIOState::SharedPtr iostate)
{
    ...
    if(iostate_buffer == 254 && L_iostate_ == 255){
        ...
        xarm_record_executor->replay_recorded_path_camera(sequence_dir_);
...
// 1 : Teaching Mode(매뉴얼 모드) 시작, 2 : 현재 상태 저장, 3 : 마지막으로 저장된 상태 삭제, 4 : 경로 계획, 5 : Teaching Mode(매뉴얼 모드) 종료, 6 : 시퀀스 변경, 7 : 카메라와 동작
bool XArmPlannerRunner::record_callback(const std::shared_ptr<xarm_msgs::srv::SetInt16Str::Request> req, std::shared_ptr<xarm_msgs::srv::SetInt16Str::Response> res)
{
    ...
    if(req->data == 1){
        ...
        auto xarm_record_executor = std::make_shared<xarm_planner::XArmPlanner>(client_node_, group_name_);
        if(xarm_record_executor->replay_recorded_path_camera(sequence_dir_)) res->ret = 1;
        else res->ret = 0;
        xarm_record_executor.reset();
    } else {
        ...
        switch(req->data){
            case 2: {
                xarm_client_planner_->get_joint_states();
                res->ret = 1;
                break;
            }
            case 3: {
            ...
            case 4: {
                ...
                xarm_client_planner_->plan_and_write(sequence_dir_);
                res->ret = 1;
                break;
            }
            case 5: {
